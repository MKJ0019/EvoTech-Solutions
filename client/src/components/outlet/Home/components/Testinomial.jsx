...
function Testinomial() {
  ...
  const Testinomial = [
    ...
    {
      id: 2,
      content: `"At EvoTech Solutions, we understand business needs beyond financial metrics, focusing on the value our solutions add to ensure the best outcomes and promising results for our clients. Our approach integrates comprehensive insights to drive success and foster long-lasting partnerships."`,
  ...
  const nextSlide = () => {
    setCurrentSlide((prev) => (prev === Testinomial.length - 1 ? 0 : prev + 1));
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev === 0 ? Testinomial.length - 1 : prev - 1));
  };
    ...
    <div className="max-w-6xl mx-auto">
      <section className="ezy__testimonial6 light py-14 md:py-24 bg-white dark:bg-[#0b1727] text-zinc-900 dark:text-white">
        <div className="container px-4 mx-auto">
          <div className="xl:w-1/2 lg:w-3/4 w-full -mb-10 mx-auto text-center">
            <svg
            ...
            <AnimatePresence initial={false} custom={direction}>
              ...
              >
                <div
                  ...
                  data-aos="zoom"
                >
                  <p className="leading-relaxed w-[50%] mb-5 text-lg">
                    {Testinomial[currentSlide].content}
                  </p>
                  <h4 className="text-gray-900 font-medium title-font tracking-wider text-lg">
                    {Testinomial[currentSlide].author}
                  </h4>
                  <p className="text-gray-500">
                    {Testinomial[currentSlide].title}
                  </p>
                  <p className="mb-6">
                    {[...Array(5)].map((_, i) => (
                      <span
                        ...
                        className={`fas fa-star ${
                          i < Math.floor(Testinomial[currentSlide].rating)
                            ? "text-yellow-500"
                            : "text-yellow-200 dark:text-opacity-20"
                        }`}
                      ></span>
                    ))}
                    {Testinomial[currentSlide].rating % 1 !== 0 && (
